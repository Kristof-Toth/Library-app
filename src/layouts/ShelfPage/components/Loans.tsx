  ...
  useEffect(() => {
    const fetchUserCurrentLoans = async () => {
      if (authState && authState.isAuthenticated) {
        ...
        const selfCurrentLoans = await fetch(url, requestOptions);

        if (!selfCurrentLoans.ok) {
          throw new Error("Something went wrong");
        }

        const selfCurrentLoansJson = await selfCurrentLoans.json();

        setShelfCurrentLoans(selfCurrentLoansJson);
      }
